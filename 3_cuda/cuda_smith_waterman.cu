...
__global__ void CalcScoreKernel(int *d_score_t2, int *d_score_t1,
                                int *d_score_t0, char *a, char *b, int *d_a_len,
                                int *d_b_len, int diag, int *d_block_score) {
  ...
  if (threadIdx.x == 0)
    d_block_score[blockIdx.x] = max(d_block_score[blockIdx.x], maxes[0]);
}

int smith_waterman(int blocks_per_grid, int threads_per_block, char *a, char *b,
                   int a_len, int b_len) {

  int *d_score_t2, *d_score_t1, *d_score_t0,
      *tmp; // score matrix in (diag, idx) coordinates
  int *d_a_len, *d_b_len;
  char *d_a, *d_b;
  ...
  for (int diag = 2; diag < a_len + b_len + 1; ++diag) {
    CalcScoreKernel<<<blocks_per_grid, threads_per_block,
                      threads_per_block * sizeof(int)>>>(
        d_score_t2, d_score_t1, d_score_t0, d_a, d_b, d_a_len, d_b_len, diag,
        d_block_score);
    tmp = d_score_t2;
  ...
  cudaMemcpy(h_block_score, d_block_score, size_block_score,
